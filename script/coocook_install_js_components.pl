...
rmtree('root/static/lib/web_js_components')
  or error("Failed removing 'root/static/lib/web_js_components'");
say colored( 'DELETE DONE.', 'green' );

say colored( 'COPY NEW JS COMPONENTS...', 'yellow' );
rcopy( 'tmp/web_js_components', 'root/static/lib/web_js_components' )
  or error("Failed at copying the new files");
